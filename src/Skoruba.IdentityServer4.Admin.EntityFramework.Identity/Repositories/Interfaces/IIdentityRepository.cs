...
namespace Skoruba.IdentityServer4.Admin.EntityFramework.Identity.Repositories.Interfaces
{
        ...
        where TUserToken : IdentityUserToken<TKey>
    {
        ...
        Task<string> UserResetTokenAsync(string userId);

        Task<IdentityResult> UserResetPasswordAsync(string userId, string token, string password);

        Task<IdentityResult> UserChangePasswordAsync(string userId, string currentPassword, string newPassword);
