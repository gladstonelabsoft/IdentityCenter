...
namespace Skoruba.IdentityServer4.Admin.EntityFramework.Identity.Repositories
{
        ...
        where TUserToken : IdentityUserToken<TKey>
    {
        ...
        public virtual async Task<PagedList<TUser>> GetUsersAsync(string search, int page = 1, int pageSize = 10)
        {
            ...
            Expression<Func<TUser, bool>> searchCondition = x => x.UserName.ToUpper().Contains(search.ToUpper()) || x.Email.ToUpper().Contains(search.ToUpper());
        ...
        {
            var user = await UserManager.Users.Where(x => x.Email.ToUpper().Trim() == email.ToUpper().Trim()).FirstOrDefaultAsync();

            return user;
        }

        public virtual async Task<TUser> GetUserByUserNameAsync(string userName)
        {
            var user = await UserManager.Users.Where(x => x.UserName.ToUpper().Trim() == userName.ToUpper().Trim()).FirstOrDefaultAsync();
        ...
        public virtual async Task<string> UserResetTokenAsync(string userId)
        {
            var user = await UserManager.FindByIdAsync(userId);
        ...
        public virtual async Task<IdentityResult> UserResetPasswordAsync(string userId, string token, string password)
        {
            var user = await UserManager.FindByIdAsync(userId);
        ...
        public virtual async Task<IdentityResult> UserChangePasswordAsync(string userId, string currentPassword, string newPassword)
        {
            var user = await UserManager.FindByIdAsync(userId);
