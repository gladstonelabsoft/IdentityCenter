...
namespace Skoruba.IdentityServer4.Admin.BusinessLogic.Identity.Services
{
        ...
        where TRoleClaimDto : RoleClaimDto<TKey>
    {
        ...
        public virtual async Task<TUserDto> GetUserAsync(string userId)
        {
            ...
            var userDto = await ValidateAndMapUserDto(user: identity);

            if(userDto is null)
            {
                throw new UserFriendlyErrorPageException(
                    message: string.Format(IdentityServiceResources.UserDoesNotExist().Description, userId),
                    errorKey: IdentityServiceResources.UserDoesNotExist().Description);
            }

            return userDto;
        }

        public virtual async Task<TUserDto> GetUserByEmailAsync(string email)
        {
        ...
        public virtual async Task<TUserDto> GetUserByUserNameAsync(string userName)
        {
        ...
        private async Task<TUserDto> ValidateAndMapUserDto(TUser user)
        {
            if (user == null)
            {
                return null;
        ...
        public virtual async Task<string> UserResetTokenByEmailAsync(string email)
        {
            var user = await IdentityRepository.GetUserByEmailAsync(email);
            if (user == null) throw new UserFriendlyErrorPageException(string.Format(IdentityServiceResources.UserEmailDoesNotExist().Description, email), IdentityServiceResources.UserEmailDoesNotExist().Description);

            var token = await IdentityRepository.UserResetTokenAsync(user.Id.ToString());
            var code = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(token));

            await AuditEventLogger.LogEventAsync(new UserPasswordResetTokenGeneratedEvent(user.UserName));

            return code;
        }

        public virtual async Task<IdentityResult> UserResetPasswordByEmailAsync(string email, string password, string code)
        {
            var user = await IdentityRepository.GetUserByEmailAsync(email);
            if (user == null) throw new UserFriendlyErrorPageException(string.Format(IdentityServiceResources.UserEmailDoesNotExist().Description, email), IdentityServiceResources.UserEmailDoesNotExist().Description);

            var token = Encoding.UTF8.GetString(WebEncoders.Base64UrlDecode(code));
            var identityResult = await IdentityRepository.UserResetPasswordAsync(user.Id.ToString(), token, password);

            await AuditEventLogger.LogEventAsync(new UserPasswordResetedEvent(user.UserName));

            return HandleIdentityError(identityResult, IdentityServiceResources.UserResetPasswordFailed().Description, IdentityServiceResources.IdentityErrorKey().Description, email);
        }

        public virtual async Task<IdentityResult> UserChangePasswordByEmailAsync(string email, string currentPassword, string newPassword)
        {
            var user = await IdentityRepository.GetUserByEmailAsync(email);
            if (user == null) throw new UserFriendlyErrorPageException(string.Format(IdentityServiceResources.UserEmailDoesNotExist().Description, email), IdentityServiceResources.UserEmailDoesNotExist().Description);

            var identityResult = await IdentityRepository.UserChangePasswordAsync(user.Id.ToString(), currentPassword, newPassword);

            await AuditEventLogger.LogEventAsync(new UserPasswordConfirmedAndChangedEvent(user.UserName));

            return HandleIdentityError(identityResult, IdentityServiceResources.UserChangePasswordFailed().Description, IdentityServiceResources.IdentityErrorKey().Description, email);
        }
