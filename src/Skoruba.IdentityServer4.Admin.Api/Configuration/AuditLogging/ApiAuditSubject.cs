...
namespace Skoruba.IdentityServer4.Admin.Api.AuditLogging
{
    public class ApiAuditSubject : IAuditSubject
    {
        public ApiAuditSubject(IHttpContextAccessor accessor, AuditLoggingConfiguration auditLoggingConfiguration)
        {
            ...
            SubjectIdentifier = subClaim == null ? (clientIdClaim == null ? "Anonymous" : clientIdClaim.Value) : subClaim.Value;
            SubjectName = subClaim == null ? (clientIdClaim == null ? "Anonymous" : clientIdClaim.Value) : nameClaim?.Value;
